...
/*
	...
	* c是世纪数减1，也就是年份的前两位。
	* y是年份的后两位。
	* m是月份。 m的取值范围是3-14， 因为某年的1、2月要看作上一年的13、14月
	* d是日数
	* [] 是取整
	！！！！！！！！！！！！！！！D小于0时，   加7的倍数  直到D大于0
*/
...
//例子
/*
计算 1994 年 12 月 13 日是星期几。显然 c = 19，y = 94，m = 12，d = 13，带入公式:
	D = [19/4] - 2*19 + 94 + [94/4] + [13*(12+1)/5] + 13 -1
	  = 4 - 38 + 94 + 23 + 33 + 12
	  = 128
	W = 128 mode 7 = 2 
*/
...
// 精度
// setprecision是保留几位有效数字 要是用小数表示要在前面加fixed
/*
double x = 12.3;
cout << x << endl;
...
int main() {
	A12* a = nullptr;  // 构造对象a时，因为为空，所以无法在栈内分配空间，因此指向虚函数表的指针就不存在，无法调用f2()
	//A12* a = new A12();
	a->f1();
	a->f2();
